...
function brukar_client_oauth_request() {
  ...
  if (count($token) > 0) {
    ...
    // Unable to retrive Rrequest token
    $debug_data = array(
      'request_uri' => request_uri(),
      'Request token http_response_header' => $http_response_header,
      'session' =>  $_SESSION,
      'token' => $token);
    watchdog(
      'brukar_client',
      'Unable to retrieve reqest token for login.<br/>Debug data:<br/><pre>!debug_data</pre><br/>',
...
function brukar_client_oauth_callback() {
  ...
  if (isset($_SESSION['auth_oauth']) && $_SESSION['auth_oauth']['oauth_token'] == $_GET['oauth_token']) {
    ...
    parse_str(trim(file_get_contents($req->to_url())), $token);
    $orig_sesion_token = $_SESSION['auth_oauth'];
    ...
    if (count($token) > 0) {
      ...
      // Unable to retrive Access token
      $debug_data = array(
        'token' => $token,
        'request_uri' => request_uri(),
        'access token http_response_header' => $http_response_header,
        'session' =>  $_SESSION,
        'session_oauth' => $orig_sesion_token);
      watchdog(
        'brukar_client',
        'Access token error.<br/>Debug data:<br/><pre>!debug_data</pre><br/>',
        array( '!debug_data' =>  print_r($debug_data, TRUE) ),
        WATCHDOG_ERROR
      );
    }
    ...
    watchdog(
      'brukar_client',
      'Request token error.<br/>Debug data:<br/><pre>!debug_data</pre><br/>',
      array( '!debug_data' =>  print_r($debug_data, TRUE) ),
      WATCHDOG_ERROR);
  }

  watchdog('brukar_client', 'Noe gikk feil under innlogging.', array(), WATCHDOG_ERROR);
...
function brukar_client_login($data) {
  ...
  if ($user->uid != 0) {
    ...
    watchdog('brukar_client', 'User already loged in, redirecting to front.', array(), WATCHDOG_DEBUG);
    drupal_goto('user');
  }
  ...
  if ($authmap_user === FALSE) {
    ...
    user_set_authmaps($user, array('authname_brukar' => $data['id']));
    watchdog('brukar_client', 'User authmap mapping created:<nr/><pre>!debug_data</pre><br/>', array('!debug_data' => print_r(array('user_id' => $user->uid, 'brukar_ident' => $data['id']), TRUE)), WATCHDOG_INFO);
  }
  else {
